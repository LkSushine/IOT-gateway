...
namespace gateway
{
	namespace common
	{
		class VibrationTransmitterPlugin
		{
		public:
			/*
			*功能：初始化协议插件
			...
			*/
			virtual ErrorInfo Init(const gateway::common::TaskInfo &taskInfo, const gateway::common::SystemInfo &sysInfo,const std::vector<gateway::common::ChannelInfoDef> &channelInfos, const std::map<std::string, PluginParam> &pluginCfg, const std::shared_ptr<spdlog::logger> &logPtr) = 0;
			...
			* CollectErrorInfo.errType;错误类型枚举
			*/
			virtual ErrorInfo PushToProcessQueue(const std::vector<ChannelInfoDef> &tagInfos, const std::vector<TagValue> &tagValues) = 0;
