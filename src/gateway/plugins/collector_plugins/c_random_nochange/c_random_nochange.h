...
class RandomActivePlugin:public gateway::common::CollectorPlugin
{
public:
	...
	{
		return std::make_shared<RandomActivePlugin>();
	}

private:
	void InitTags(const std::vector<gateway::common::TagInfo> &tagInfos);

	std::map<int, gateway::common::TagInfo> tagValueMap_;
...
BOOST_DLL_ALIAS(
	RandomActivePlugin::CreateCollectorPlugin,
